...
const CHECKPOINTS_DIR = join(SPAR_DIR, 'checkpoints');

function ensureDirectories() {
    [SPAR_DIR, PERSONAS_DIR, SESSIONS_DIR, CHECKPOINTS_DIR].forEach(dir => {
...
async function debateStart(decision, options) {
    ...
    const session = {
        ...
        responses: { round1: {}, round2: {}, round3: {} },
        synthesis: null,
        interrogation: {},
        finalPositions: {}
    };
    ...
    console.log(chalk.gray(formatConfigSummary(DEFAULT_DEBATE_CONFIG)));

    const r1Budget = TOKEN_BUDGETS.round1_opening;
    const announcePrompt = `THE DECISION: ${decision}

Analyze this decision from your perspective (${r1Budget.promptGuidance}):
...
- What would change your mind?`;
    ...
    console.log(chalk.gray(`  Token budget: ${TOKEN_BUDGETS.round2_response.maxTokens} tokens`));
    ...
    const r2Budget = TOKEN_BUDGETS.round2_response;
    const clashPrompt = `The other perspectives said:\n\n${clashContext}\n\nWhere do you DISAGREE with them? What are they missing? (${r2Budget.promptGuidance})`;
    ...
    const round2Promises = selectedPersonas.map(async (id) => {
        ...
        if (!persona) return;
        try {
            const response = await callAI(provider, apiKey, persona.prompt, clashPrompt, {
                model, baseUrl, maxTokens: r2Budget.maxTokens
            });
    ...
    // Round 3 - Final Positions (NEW in v4.0)
    console.log(chalk.bold.red(`\n═══ RUMBLE — Round 3/3: Final Positions ═══\n`));
    console.log(chalk.gray(`  Token budget: ${TOKEN_BUDGETS.round3_final.maxTokens} tokens`));

    const allResponses = selectedPersonas.map(id => {
        const p = allPersonas[id];
        const r1 = session.responses.round1[id] || '';
        const r2 = session.responses.round2[id] || '';
        return `${p?.name || id} (Opening): ${r1.substring(0, 200)}...\n${p?.name || id} (Clash): ${r2.substring(0, 200)}...`;
    }).join('\n\n');

    const r3Budget = TOKEN_BUDGETS.round3_final;
    const finalPrompt = `Having heard the full debate:\n\n${allResponses}\n\nProvide your FINAL POSITION (${r3Budget.promptGuidance}):
- Has anything changed your view?
- What's your final verdict on this decision?
- What one thing must the decision-maker understand?`;

    for (const id of selectedPersonas) {
        spinners[id] = ora({ text: `${allPersonas[id]?.icon} final position...` }).start();
    }

    const round3Promises = selectedPersonas.map(async (id) => {
        const persona = allPersonas[id];
        if (!persona) return;
        try {
            const response = await callAI(provider, apiKey, persona.prompt, finalPrompt, {
                model, baseUrl, maxTokens: r3Budget.maxTokens
            });
            session.responses.round3[id] = response;
            spinners[id].succeed();
        } catch (error) {
            session.responses.round3[id] = `Error: ${error.message}`;
            spinners[id].fail();
        }
    });
    ...
    for (const id of selectedPersonas) {
        ...
        const colorFn = colorFns[id] || chalk.white;
        console.log(colorFn(`\n┌${'─'.repeat(70)}┐`));
        console.log(colorFn(`│ ${persona.icon}  ${persona.name} — FINAL POSITION${' '.repeat(42)} │`));
        console.log(colorFn(`└${'─'.repeat(70)}┘`));
        console.log(chalk.white('\n' + (session.responses.round3[id] || '').split('\n').map(l => '   ' + l).join('\n')));
    }
    ...
    console.log(chalk.bold.cyan('\n═══ STEP 5: KNIT — Synthesis ═══\n'));
    console.log(chalk.gray(`  Token budget: ${TOKEN_BUDGETS.knit_synthesis.maxTokens} tokens (Supervisor tier)`));

    const synthSpinner = ora('Synthesizing tensions...').start();
    try {
        const synthPrompt = `Synthesize this complete 3-round SPAR debate on: ${decision}
...
${JSON.stringify(session.responses.round3, null, 2)}

Provide a comprehensive synthesis:
1. KEY TENSIONS — Where do personas fundamentally disagree?
2. CONVERGENCE — What do they surprisingly agree on?
3. EVOLUTION — How did positions change across 3 rounds?
4. UNEXAMINED — What has NOT been addressed?
5. DRAFT SYNTHESIS — A position that honors valid concerns from all sides`;

        const synthBudget = TOKEN_BUDGETS.knit_synthesis;
        session.synthesis = await callAI(provider, apiKey,
            'You are a neutral debate moderator synthesizing tensions from a 3-round dialectic.',
            synthPrompt, { model, baseUrl, maxTokens: synthBudget.maxTokens });
    ...
    if (runInterrogate && session.synthesis) {
        ...
        console.log(chalk.gray(`  Token budget: ${TOKEN_BUDGETS.interrogate.maxTokens} tokens`));

        const interrogateBudget = TOKEN_BUDGETS.interrogate;
        const interrogatePrompt = `The moderator has proposed this synthesis:

${session.synthesis}

Evaluate this synthesis (${interrogateBudget.promptGuidance}):
...
- What would make this synthesis FAIL?`;
        ...
        for (const id of selectedPersonas) {
            ...
            try {
                session.interrogation[id] = await callAI(provider, apiKey, persona.prompt, interrogatePrompt, {
                    model, baseUrl, maxTokens: interrogateBudget.maxTokens
                });
                spinner.succeed();
    ...
    console.log(chalk.gray(`  Token budget: ${TOKEN_BUDGETS.transmit.maxTokens} tokens`));

    const transmitSpinner = ora('Generating actionable recommendations...').start();
    try {
        const transmitBudget = TOKEN_BUDGETS.transmit;
        const transmitPrompt = `Based on this complete 3-round SPAR session:
...
Provide (${transmitBudget.promptGuidance}):
...
5. NEXT ACTION — The very next step to take`;
        ...
        const finalRec = await callAI(provider, apiKey,
            ...
            transmitPrompt, { model, baseUrl, maxTokens: transmitBudget.maxTokens });
...
function exportSessionMarkdown(session) {
    const allPersonas = getAllPersonasWithCustom();
    let md = `# SPAR Session (SPARKIT Protocol v4.0)\n\n`;
    md += `**Date**: ${session.timestamp}\n`;
    md += `**Provider**: ${session.provider} (${session.model})\n`;
    md += `**Protocol**: 3-Round Rumble\n\n---\n\n`;
    md += `## 🎯 The Decision\n\n${session.decision}\n\n---\n\n`;
    ...
    for (const id of session.personas) {
        const p = allPersonas[id];
        md += `### ${p?.icon || '•'} ${p?.name || id}\n${session.responses.round3?.[id] || '_Not run_'}\n\n`;
    }
...
// Arena commands
const arena = program.command('arena').description('🏟️ Pre-configured debate formations');

arena.command('list [category]')
    .description('List all arenas (optionally by category)')
    .action(async (category) => {
        const { arenaList } = await import('./commands.js');
        arenaList(category);
    });

arena.command('show <id>')
    .description('Show arena details and prompts')
    .action(async (id) => {
        const { arenaShow } = await import('./commands.js');
        arenaShow(id);
    });

arena.command('start <id> <decision>')
    .alias('run')
    .description('Start a debate using an arena')
    .action(async (id, decision) => {
        const { arenaStart } = await import('./commands.js');
        await arenaStart(id, decision);
    });

// Quick arena shorthand: sparkit arena feature-validation "decision"
arena.argument('[id]', 'Arena ID')
    .argument('[decision]', 'Decision question')
    .action(async (id, decision, options) => {
        if (id && decision) {
            const { arenaStart } = await import('./commands.js');
            await arenaStart(id, decision);
        } else if (id && !decision) {
            // Show arena details if only ID provided
            const { arenaShow } = await import('./commands.js');
            arenaShow(id);
        }
    });
